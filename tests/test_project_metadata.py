...
class ProjectMetadataTest(unittest.TestCase):
    ...
    def test_amiga_build_uses_native_workbench_gui(self):
        ...
        self.assertIn("struct AppPens", source)
        self.assertIn("compute_app_layout", source)
        self.assertIn("layout_gadgets", source)
        self.assertIn("draw_colored_transcript", source)
        self.assertIn("transcript_chars_per_row", source)
        self.assertIn("transcript_drawable_rows", source)
        self.assertIn("transcript_total_visual_rows", source)
        self.assertIn("SetAPen", source)
        self.assertIn("TextLength", source)
        self.assertIn("Text(rast_port", source)
        self.assertIn("CHAT_STYLE_ASSISTANT", source)
    ...
    def test_send_receive_milestone_is_implemented(self):
        ...
        self.assertIn("bridge_output_update_style", source)
        self.assertIn("value >= 193 && value <= 218", source)
        self.assertIn("value - 128", source)
        self.assertIn("C64_LIGHT_GREEN", source)
        self.assertIn("CHAT_STYLE_THINKING", source)
